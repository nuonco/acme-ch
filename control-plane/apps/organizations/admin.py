...
class OrganizationAdmin(admin.ModelAdmin):
    ...
    def nuon_install_display(self, obj):
        ...
        if obj.nuon_install:
            ...
            return mark_safe(f'<pre style="background-color: #1e1e1e; color: #d4d4d4; padding: 15px; border-radius: 6px; overflow: auto; max-height: 600px; border: 1px solid #333; font-size: 13px; line-height: 1.5;">{pretty_json}</pre>')
        return "-"
    ...
    fieldsets = [
        ...
        (
            "Nuon",
            {
                "fields": ("nuon_install_id", "reprovision_button", "nuon_install_display", "nuon_install_state_display", "nuon_install_stack_display", "nuon_workflows_display"),
                "classes": ("collapse",),
            },
        ),
    ]
